...
# Description:
#       Data wrangling for Columbian black-tailed deer in the Alsea WMU in 2014
...
# Path to Excel file
path <- "./Data/0_Raw/2014AlseaDog2014_for ODFW_6July2015.xlsx"
...
data_geo <- as.data.frame(df_list$`Alsea Dog sample info`)
data_gen <- as.data.frame(df_list$`all 2014 AlD genotype data`)
data_assn <- as.data.frame(assn_df_list$`2014 Alsea Dog-collected`)
...
data_geo <- data_geo %>%
  mutate(
    ...
    `UTM Northing (NAD 83)`            = as.character(`UTM Northing (NAD 83)`) %>% trimws()
  ) %>%
  mutate(
    # Standardize NA
    `UTM Easting (NAD 83)` = ifelse(`UTM Easting (NAD 83)` %in% c("", "NA", "na", "Na", "NULL"), NA, `UTM Easting (NAD 83)`),
    `UTM Northing (NAD 83)`            = ifelse(`UTM Northing (NAD 83)` %in% c("", "NA", "na", "Na", "NULL"), NA, `UTM Northing (NAD 83)`)
  ) %>%
  mutate(
    # To numeric
    `UTM Easting (NAD 83)` = as.numeric(`UTM Easting (NAD 83)`),
    `UTM Northing (NAD 83)`            = as.numeric(`UTM Northing (NAD 83)`)
  ) %>%
  # Remove NAs
  filter(!is.na(`UTM Easting (NAD 83)`), !is.na(`UTM Northing (NAD 83)`))
...
data_geo %>%
  summarise(
    ...
    Northing_NAs = sum(is.na(`UTM Northing (NAD 83)`))
  )

# Now easting/northing to lat/long
coords_sf <- st_as_sf( #  convert to a sf object
  data_geo,
  coords = c("UTM Easting (NAD 83)", "UTM Northing (NAD 83)"),
  crs = 26910
) 
...
saveRDS(data_merge, file = "./Data/1_YearWMU_processed/2014AlseaDog.rds")
