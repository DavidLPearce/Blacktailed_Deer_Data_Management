...
# Description:
#       Data wrangling for Columbian black-tailed deer in the Applegate WMU in 2016
...
# Path to Excel file
path <- "./Data/0_Raw/2016ApplegateDog2016.xlsx"
...
data_geo <- as.data.frame(df_list$`Applegate Dog Sample info.`)
data_gen <- as.data.frame(df_list$`All 2016 Applegate Dog Genotype`)
data_assn <- as.data.frame(df_list$`2016 ApD Deer Assignment`)
...
saveRDS(data_merge, file = "./Data/1_YearWMU_processed/2016ApplegateDog.rds")
