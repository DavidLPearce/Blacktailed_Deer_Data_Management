...
data_geo <- as.data.frame(df_list$`Indigo Dog sample info`)
data_gen <- as.data.frame(df_list$`2015 InD Genotype Data`)
data_assn <- as.data.frame(assn_df_list$`2015 Indigo dog-collected`)
...
saveRDS(data_merge, file = "./Data/1_YearWMU_processed/rds/2015IndigoDog.rds")
