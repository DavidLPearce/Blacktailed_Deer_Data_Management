...
# Description:
#       Data wrangling for Columbia black-tailed deer in the Dixon WMU in 2013
...
data_geo <- as.data.frame(df_list$`Dixon Human Coordinates`)
data_gen <- as.data.frame(df_list$`Dixon Human Genotypes`)
data_assn <- as.data.frame(assn_df_list$`2013 Dixon-dog collected`)
...
data_gen <- data_gen %>% select(-SRY, -GADPH)
head(data_gen)
...
saveRDS(data_merge, file = "./Data/1_YearWMU_processed/rds/2013DixonHuman.rds")
