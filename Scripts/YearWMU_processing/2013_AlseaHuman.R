...
# Description:
#       Data wrangling for Columbia black-tailed deer in the Alsea WMU in 2013
...
data_geo <- as.data.frame(df_list$`Alsea Deer Human Coordinates`)
data_gen <- as.data.frame(df_list$`Alsea Deer Human 16-33 17July14`)
data_assn <- as.data.frame(assn_df_list$`2013 Alsea-human collected`)
...
data_gen <- data_gen %>% select(-SRY, -GAPDH)
head(data_gen)
...
saveRDS(data_merge, file = "./Data/1_YearWMU_processed/rds/2013AlseaHuman.rds")
