...
# Description:
#       Data wrangling for Columbia black-tailed deer in the Trask WMU in 2014
...
# Path to Excel file
path <- "./Data/0_Raw/2014TraskHuman2014_forODFW_10July15.xlsx"
...
data_geo <- as.data.frame(df_list$`Trask Human sample info`)
data_gen <- as.data.frame(df_list$`2014 TrH genotype data`)
data_assn <- as.data.frame(assn_df_list$`2014 Trask Human-collected`)
...
data_geo <- data_geo %>%
  mutate(
    ...
    `UTM_E` = as.character(`UTM_E`) %>% trimws(),
    `UTM_N`            = as.character(`UTM_N`) %>% trimws()
  ) %>%
  mutate(
    # Standardize NA
    `UTM_E` = ifelse(`UTM_E` %in% c("", "NA", "na", "Na", "NULL"), NA, `UTM_E`),
    `UTM_N`            = ifelse(`UTM_N` %in% c("", "NA", "na", "Na", "NULL"), NA, `UTM_N`)
  ) %>%
  mutate(
    # To numeric
  ...
  # Remove NAs
  filter(!is.na(`UTM_E`), !is.na(`UTM_N`))
...
data_geo %>%
  summarise(
    Easting_NAs  = sum(is.na(`UTM_E`)),
    Northing_NAs = sum(is.na(`UTM_N`))
  )
...
saveRDS(data_merge, file = "./Data/1_YearWMU_processed/rds/2014TraskHuman.rds")
