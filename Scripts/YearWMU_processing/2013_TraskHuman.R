...
# Description:
#       Data wrangling for Columbian black-tailed deer in the Trask WMU in 2013
...
data_geo <- as.data.frame(df_list$`Trask Human coordinates`)
data_gen <- as.data.frame(df_list$`Trask Human genotypes`)
data_assn <- as.data.frame(assn_df_list$`2013 Trask-human collected`)
...
data_gen <- data_gen %>% select(-GAPDH.1, -GAPDH.2)
head(data_gen)
...
saveRDS(data_merge, file = "./Data/1_YearWMU_processed/2013TraskHuman.rds")
