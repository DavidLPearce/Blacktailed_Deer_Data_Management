...
# Description:
#       Data wrangling for Columbia black-tailed deer in the Indigo WMU in 2012
...
# Path to Excel file
path <- "./Data/0_Raw/2012 Indigo genos_26June19.xlsx"
...
data_gen <- as.data.frame(df_list$`2012 Indigo genos-ALL`)
data_assn <- as.data.frame(assn_df_list$`2012 Indigo`)
...
data_gen <- data_gen %>% select(-GENDER.1, -GENDER.2)
head(data_gen)
...
saveRDS(data_merge, file = "./Data/1_YearWMU_processed/rds/2012Indigo.rds")
