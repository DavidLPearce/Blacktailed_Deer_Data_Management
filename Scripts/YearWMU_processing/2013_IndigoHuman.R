...
# Description:
#       Data wrangling for Columbian black-tailed deer in the Indigo WMU in 2013
...
# Path to Excel file
path <- "./Data/0_Raw/2013 Deer Data for PME_with_locations_.xlsx"
...
assn_path <- "./Data/0_Raw/2013 Deer Matching.xlsx"
...
data_geo <- as.data.frame(df_list$`Indigo Human Coordinates`)
data_gen <- as.data.frame(df_list$`Indigo Human genotypes`)
data_assn <- as.data.frame(assn_df_list$`2013 Indigo-human collected`)
...
data_gen <- data_gen %>% select(-SRYGADPH.1, -SRYGADPH.2)
head(data_gen)
...
data_geo$`UTM (E, N)` <- gsub("\\.", ",", data_geo$`Node  UTM (E, N)`) # replacing periods with comma
data_geo$Easting <- as.numeric(sub(",.*", "", data_geo$`Node  UTM (E, N)`))
...
data_merge <- data_gen %>%
  left_join(
    data_assn %>% select(`ODFW Sample Name`, `Deer Assignment`, Sex),
...
saveRDS(data_merge, file = "./Data/1_YearWMU_processed/2013IndigoHuman.rds")
