...
data_gen <- as.data.frame(df_list$`All 2015 Trask Dog Genotypes`)
data_assn <- as.data.frame(assn_df_list$`2015 Trask dog-collected`)
...
data_merge <- data_gen %>%
  left_join(
    ...
    by = c("OSU sample" = "OSU Sample Name")
  )%>%
  # Merge Latitude and Longitude from data_geo - and ODFW ID
  left_join(
    data_geo %>% select(`OSU label`, Latitude, Longitude, "ODFW Sample #"),
    by = c("OSU sample" = "OSU label")
  )%>%
...
saveRDS(data_merge, file = "./Data/1_YearWMU_processed/rds/2015TraskDog.rds")
